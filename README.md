...
- A～Dの4選手を固定表示する配信用画面
- 個人戦とタッグ戦（7チーム・14名）の大会形式選択
- タッグ戦の総当たり21試合、勝点順位、準決勝・決勝、サドンデス管理
- 予選同点時のサドンデス対象表示
...
大会作成時に「タッグ戦（7チーム・14名）」を選択します。タッグ戦のチーム・対戦・結果データは既存の `tournaments.draw_schedule` JSON内へ保存するため、今回の機能追加に伴うDB変更やSQLの再実行は不要です。

1. 「チーム」から7チーム・各2名と、重複しない☆12選曲カード8曲を登録します。
2. 「対戦表」で7ラウンド・全21試合の総当たり表を作成します。
3. 「試合結果」で新曲戦と☆12戦の出場選手・選曲・勝敗を入力します。
4. 全チームが6試合を完了し、必要なサドンデスを確定したら準決勝を作成します。
5. 準決勝2試合の勝者確定後に決勝を作成します。決勝の☆12戦では決勝用BOXを選択できます。
